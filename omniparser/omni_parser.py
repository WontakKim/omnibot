...
class OmniParser:
    def __init__(self, config: Dict):
        self.ocr_adapter = EasyOCRAdapter(['en', 'ko'])
        self.object_detection_adapter = YoloAdapter('../weights/icon_detect/model.pt')
        self.vision_language_adapter = FlorenceAdapter('../weights/icon_caption_florence')

    def parse(self, image: Union[str, Image.Image]) -> Tuple[str, List[SOM]]:
        if isinstance(image, str):
            image = Image.open(image)
        som_labeled_elements = self.get_som_labeled_element(image)
        annotated_image = create_annotated_image(image, som_labeled_elements)
        annotated_image.show()

        buffered = io.BytesIO()
        annotated_image.save(buffered, format='JPEG')
        encoded_image = base64.b64encode(buffered.getvalue()).decode('ascii')
        return encoded_image, som_labeled_elements
    ...
    def get_som_labeled_element(self, image: Image.Image, use_local_semantics: bool=True) -> List[SOM]:
        ...
        if use_local_semantics:
            ...
            if len(no_labeled_elements) > 0:
                bboxes = [element.bbox for element in no_labeled_elements]
                # create caption text
    ...
    def _create_labeled_element(obj_bboxes, ocr_bboxes, ocr_texts) -> List[SOM]:
        ...
        ocr_match_indices, obj_match_indices = np.where(inclusions)
        use_ocr_indices = set(ocr_match_indices)
